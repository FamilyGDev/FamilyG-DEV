	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('wine_clothes_wine'), value = 'brinks_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'brinks_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 336.17, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
				end)
			end,
...
AddEventHandler('esx_brinks:hasEnteredMarker', function(zone)
	if zone == 'PieceFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks'  then
		CurrentAction     = 'piece_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end	

	if zone == 'TraitementBillet' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks'  then
		CurrentAction     = 'billet_traitement'
		CurrentActionMsg  = _U('press_traitement_raffine')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'TraitementSacnumerotter' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks'  then
		CurrentAction     = 'sacnumerotter_traitement'
	...
	if zone == 'BrinksActions' and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
		CurrentAction     = 'brinks_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_brinks:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementBillet') and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
		TriggerServerEvent('esx_brinks:stopTransform')
	end
	if (zone == 'TraitementSacnumerotter') and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' then
		TriggerServerEvent('esx_brinks:stopTransform')
	end
...
--		local blip = AddBlipForCoord(Config.Zones.BrinksActions.Pos.x, Config.Zones.BrinksActions.Pos.y, Config.Zones.BrinksActions.Pos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'brinks' and (GetGameTimer() - GUI.Time) > 150 then
...
RegisterNetEvent('NB:openMenubrinks')
AddEventHandler('NB:openMenubrinks', function()
	OpenMobilebrinksActionsMenu()
end)